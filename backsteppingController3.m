function delta = backsteppingController3(state, parameters)
% Working
    Cf = parameters.Cf;
    Cr = parameters.Cr;
    lf = parameters.lf;
    ...
    alpha = (-state(1)^3 - Vx * sin(state(3))) / cos(state(3));

    alphaDot = -(3 * state(1)^2 * yglobalDot * cos(state(3)) + state(1)^3 * sin(state(3)) * state(4)) / ...
